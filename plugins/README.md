...
| Directory | Purpose |
|-----------|---------|
| `sec-overlay/` | Agentic security-audit harness; SAST + multi-agent investigation. Python core is the `sec_overlay` package under `skills/sec-overlay/helpers/`; run instructions resolve paths from `${CLAUDE_PLUGIN_ROOT}`. Test suite green after the rename. |
