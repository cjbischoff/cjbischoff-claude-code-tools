...
| **2-4 analysis** | recon → architecture → arch-gate → threat-model → tm-gate | `injection` lands on the prioritized hunt list because recon saw Flask + raw SQL; opus phase-adversary confirms the entrypoint claim resolves to real code. | `kb/scan-profile.json`, `threat-model/threat-model.md` |
