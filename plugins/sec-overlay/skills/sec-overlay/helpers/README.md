...
| Module | Purpose |
|--------|---------|
| `calibrate.py` | Deterministic 1–10 `risk_score` for confirmed / needs-deployment-testing findings (severity base map + class boost + baseline cap + precondition gates). `PRECONDITION_CAPS` (threshold, cap) pairs and `PRECONDITION_CAP_FLOOR` are the single source `_precondition_cap` reads; `prompt-constants.md`'s `SEVERITY_PRECONDITION` block states the same table (REQ-32 contract lint). CLI-callable. |
...
| `review_coverage.py` | `CoverageManifest` — per-file review coverage (`pending` → `in_review` → `done`/`failed`) persisted to `artifacts/coverage_manifest.json` after every transition. `seal()` returns `complete` only when every entry is `done`, `partial` when some are `failed`, and **raises** `CoverageTransitionError` if any entry is still `pending`/`in_review` — a run must never claim coverage it did not perform. Only this class edits the manifest JSON. `MANIFEST_VERSION` is 2 (SCALE-03): the manifest also carries `model`/`profile`, keyword-only on the constructor and round-tripped through `to_dict`/`load`; a version-1 manifest with no such keys loads both as `None`. `check_resume_identity(prior, *, model, profile)` raises `ResumeIdentityError` — naming both values — when the current run's `model` or `profile` differs from a prior manifest's recorded value; a prior value of `None` permits anything (never enforced retroactively). `cli.run_review` loads any existing manifest and runs this check before resolving refs or writing anything, so a resumed run that switched model/profile is rejected (exit 2) with the on-disk workspace left byte-identical. |
...
| `review_agent.py` | `render_review_prompt(path, rule_text, diff, changed_files)` — renders `agents/review-file.md` for one file's review pass. `parse_review_response(text, *, path, rule_id_prefix, bundle_paths=None)` — the elevation-of-privilege backstop (REV-03): converts a `code_comment`/`task_done` JSON tool-call array into `Finding`s, discarding (never converting) a comment naming a path outside the reviewing unit's membership (Strict Focus Rule, enforced mechanically). Membership is `{path}` when `bundle_paths` is `None` (single-file behavior, unchanged); when a `ReviewUnit`'s member set is supplied, a comment naming any member is kept and attributed to *that entry's own path* — not the outer `path` — so a multi-file unit's comments land on the right file. `recorded_return_source(ws, *, base, head, bundle_paths_by_path=None)` looks up each file's membership in that map and passes it through. Every finding still carries `REVIEW_AGENT_CLAIM` (`evidence.as_llm_claim("review-agent")`) as its sole evidence source and `FindingStatus.RAW`, both assigned in code — never read from the model's response. No network/subprocess import (D-13) — dispatch stays in `SKILL.md`. REQ-P3 adds the per-file plan seam: `render_plan_prompt(path, rule_text, diff, *, repo_root, overlay_root)` renders `agents/review-plan.md`, `plan_agent_label(path)` labels its dispatch, `plan_guidance_from_return(text)` parses the plan agent's strict JSON into severity-ordered guidance (raising `ValueError` on malformed input), and `render_review_prompt` gains a keyword-only `plan_guidance` that fills the review prompt's `{{PLAN_GUIDANCE}}` token. `PLAN_LINE_THRESHOLD = 100` gates which units get a plan pass. REQ-P8 adds a keyword-only `background` argument: when non-empty its text is wrapped in a `background-context` untrusted envelope (`envelope.wrap_untrusted`) and fills the review prompt's `{{BACKGROUND}}` token; empty background leaves no envelope. |
| `background.py` | REQ-P8's background-context sanitizer. `load_background(text=None, *, path=None) -> str` accepts exactly one source (else `ValueError`) and runs five guards in order: reject over a 1 MB `BACKGROUND_MAX_BYTES` cap (`ValueError`), strip C0/C1 control characters (newline and tab kept), neutralize envelope delimiters (`envelope.neutralize_markers`), hard-abort on any detected secret (`redactor.verify_no_secrets`, run BEFORE `safe_for_prompt` so a maskable token still aborts rather than passing silently), then `redactor.safe_for_prompt` for defense in depth. `cli.run_review` gains a `background` kwarg; the `review` CLI exposes mutually-exclusive `--background`/`--background-file` flags that exit 2 on an oversized or secret-bearing payload. Stdlib + `redactor`/`envelope` only. |
...
| `pr_poster.py` | REQ-S1's stdlib GitHub pull-request review poster. `route_findings(findings) -> (inline, summary)` splits critical/high (inline comments) from everything else (summary body). `build_review_payload(findings) -> dict` builds the review payload (`event` always `COMMENT`, `body` markdown, `comments` inline list). `post_review(*, owner, repo, pull_number, token, payload, transport=_urllib_transport)` POSTs to `/repos/{owner}/{repo}/pulls/{n}/reviews` with bearer auth; `transport` is injectable for tests. `main(argv)` reads `GITHUB_TOKEN`/`GITHUB_REPOSITORY` from the env and takes `<review_result.json> <pull_number>` from argv. Stdlib-only (`urllib`); the composite `action.yml` at the plugin root invokes it. Event is `COMMENT` so the action never blocks a merge on its own. CLI-callable. |
| `sessions.py` | REQ-S2's read-only renderer over the per-repo sidecar. `session_rows(sessions_root)`/`render_rows` list one row per `<slug>/` (pass, sha, finding counts read from `artifacts/review_result.json`). `resolve_session(sessions_root, id)` resolves `"latest"` by `state.json` mtime or an exact slug (`KeyError` on miss). `session_detail(session_dir, *, severity)`/`render_detail` show stages plus a ledger summary (`position_reviews`/`dropped` counts from `artifacts/review_ledger.json`) and a `--severity`-filtered finding list. Never writes. Reached via `cli.py`'s `sessions list|show` subcommand, not `python -m`. |
| `review_result.py` | REQ-P7's consolidated per-run artifact. `write_review_result(ws, *, findings, dropped, declines, retractions, skips, manifest, budget_exceeded, tokens, base, head, model, profile, tier) -> Path` writes `artifacts/review_result.json` through `workspace._atomic_write`, mirroring the OCR `output.go` record. Keys (all always present, `RESULT_KEYS`): `status` (the manifest seal, read from `manifest.to_dict()["seal"]` — never re-sealed, so a zero-finding empty manifest records `null` instead of raising), per-finding `findings` (each `id`/`path`/`line`/`severity`/`rule_id`/`profile`/`disposition`), `dropped`/`declined`/`retractions`/`skips` (dataclasses serialized via `asdict`, dicts passed through), `budget_exceeded`, `coverage_manifest` (the same `to_dict()`), `tokens` (per-phase records; review mode has no live token tracking, so `cli.run_review` passes `{}`), `base`/`head`, `model`/`profile`/`tier` (`tier` is `None` until the Milestone-4 tiers land). Additive — every prior artifact stays. `cli.run_review` calls it last on both consume exits (zero-reviewable and post-seal). `RESULT_FILENAME = "review_result.json"`. |
...
| Module | Command does |
|--------|--------------|
| `cli` | `scan` (deterministic prefilter → SARIF/MD), `memory` (status / append a learning), `audit` (deterministic audit driver), `sessions list\|show` (read-only render over the sidecar via `sessions.py`; `list --target`, `show <id\|latest> --target [--severity]`), `rules check <path> --root <path>` (prints the resolved rule layer + doc for one path via `rule_glob.resolve_with_layer`; a misspelled top-level subcommand names the nearest valid one through the `_SuggestingParser`/`difflib` did-you-mean hook), and `review --base <ref> --head <ref> --root <path> --profile security\|general` (diff-scoped, position-verified review pass — tracer path, one changed file end to end; `run_review` always calls `report.write_report` with the gate's dropped/declined output, so `report.md` and `artifacts/review_ledger.json` carry every run's drop/decline sections, including the zero-drop/zero-decline case; the position gate's kept findings are then run through `review_findings.apply_profile(..., profile)` — a `ValueError` from an unknown profile or gate marking exits 2, same as a bad ref). Like `scan` and `audit`, `review` resolves this output under the per-repo sidecar (`RepoMemory.for_target(root).workspace`, i.e. `<root>/.sec-overlay/<slug>/`), never at `--root` itself — `review` now takes its own `--workspace` override too, mirroring `scan`/`audit`, so the same value (the default sidecar or an explicit `--workspace`) must be passed to every invocation of one run. `review` also takes `--concurrency` (default 8, 1-128), `--timeout` (default 600 seconds, 1-3600), and `--max-git-procs` (default 16, 1-128), each validated by `_bounded_int` before any git subprocess runs — an out-of-range value exits 2 naming the flag and its range, never silently clamped (`grep -c 'max(1, min(' sec_overlay/cli.py` stays 0). `--concurrency` bounds `SKILL.md`'s agent dispatch fan-out; the Python core never dispatches an agent itself, so this flag has no enforcement point in `cli.py` beyond validating the bound and recording it in `SKILL.md`'s dispatch step. The diff-line-count comprehension in `run_review` dispatches through `_bounded_map(items, workers, fn)`, a `ThreadPoolExecutor` sized to `min(max_git_procs, len(items))` and consumed via order-preserving `.map()` (never `as_completed()`), so results are always positionally aligned with the input regardless of which worker finishes first; an empty `items` never constructs a pool. The manifest-building loop instead dispatches per SCALE-01 `ReviewUnit` (`group_bundles(selection.reviewable)`, retained as `units`): each unit's member paths are fetched together by `_fetch_review_unit_files`, submitted via `ex.submit(...)` (one `ThreadPoolExecutor` sized to `min(max_git_procs, len(units))`) and read back with `future.result(timeout=timeout)` in submission order — never `as_completed()`. `_fetch_file_review_inputs` still runs one file's three git calls on a worker thread and catches its own exception per file, so an ordinary per-file failure only fails that file; a unit whose combined fetch work exceeds `--timeout` raises `TimeoutError` from `.result()`, and every one of that unit's member paths is then failed with the fixed note `TIMEOUT_NOTE` ("review unit exceeded --timeout"), sealing the manifest `"partial"` (rc 3) rather than raising. The consuming (main) thread still performs every `manifest.add`/`start`/`finish`/`fail` transition in `selection.reviewable`'s original file order, looked up from the per-unit fetch results by path, exactly as before parallelization. A resumed run (an existing `coverage_manifest.json` at the workspace, past the identity check) reads `base_sha`/`head_sha` from that prior manifest instead of resolving the CLI's `--base`/`--head`, then round-trips each persisted SHA through `resolve_ref_sha` — a HEAD that moved since the prior run never changes what a resumed run reads, and a persisted SHA that no longer resolves (rewritten/GC'd) exits 2 via the same unresolvable-ref path rather than reading a different tree as an empty diff (SCALE-03, T-04-12). The shared runner `r` also binds `cwd=root` (05-01 tracer fix) — `diffscope.py`'s git calls carry no `-C <path>`, so without this binding every diff/rev-parse call ran against whatever directory the CLI process happened to be launched from, silently reviewing the wrong repo (or none) when invoked from outside `--root`; an injected `runner=` (tests) is unaffected. |
