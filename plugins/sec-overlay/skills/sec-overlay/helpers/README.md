...
| `review_agent.py` | `render_review_prompt(path, rule_text, diff, changed_files)` — renders `agents/review-file.md` for one file's review pass. `parse_review_response(text, *, path, rule_id_prefix)` — the elevation-of-privilege backstop (REV-03): converts a `code_comment`/`task_done` JSON tool-call array into `Finding`s, discarding (never converting) a comment naming a different path (Strict Focus Rule, enforced mechanically). Every finding it produces carries `REVIEW_AGENT_CLAIM` (`evidence.as_llm_claim("review-agent")`) as its sole evidence source and `FindingStatus.RAW`, both assigned in code — never read from the model's response — so `evidence.confirms_alone` is always false for an agent-authored finding. No network/subprocess import (D-13) — dispatch stays in `SKILL.md`. |
| `bundle.py` | SCALE-01's grouping unit. `ReviewUnit(unit_id, files)` (frozen, `files` non-empty) + `group_bundles(reviewable: list[ChangedFile]) -> list[ReviewUnit]`, called from `cli.run_review` on `selection.reviewable` — downstream of `file_select.partition`, never on raw `changed_file_records`. This plan's grouping is degenerate — one unit per file, order preserved — so a single-file run stays byte-identical to pre-bundling behavior; real multi-file grouping is a later plan. Pure and total: no filesystem/subprocess/`Workspace` import. `_unit_id` reuses the `_stable_finding_id` truncated-sha256 idiom. |
| `review_comments.py` | OUT-01's diff-anchored comment writer. `DiffComment(path, line, side, existing_code, content)` (frozen) + `comment_from_finding(finding) -> DiffComment` (`path←finding.file`, `line←finding.line`, `side←DEFAULT_SIDE`, `existing_code←finding.evidence`, `content←finding.message`) + `write_review_comments(ws, comments, manifest_dict) -> Path`, writing `artifacts/review_comments.json` (`{"comments": [...], "coverage_manifest": manifest_dict}`) through `workspace._atomic_write`. `DEFAULT_SIDE = "RIGHT"` is the module's only literal `"RIGHT"`; `cli.run_review` calls this once, after `write_report`, with `CoverageManifest.to_dict()`'s own dict — the embedded manifest is the same object the seal decision reads from. |
