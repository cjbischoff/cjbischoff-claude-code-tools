...
| `file_select.py` | `partition(records)` — split `diffscope.ChangedFile` records into `reviewable` and `excluded` (deleted / generated / not-allowlisted, from the closed `EXCLUSION_REASONS` vocabulary), checked in that order. `ALLOWED_EXTENSIONS` is the full 86-extension allowlist ported from open-code-review; `DEFAULT_EXCLUDE_GLOBS` is 40 fnmatch patterns (brace-expanded from OCR's 34) driving `_is_generated`. `_normalize_path` undoes git's octal-escape quoting of non-ASCII paths before matching. Path-shaped, not finding-shaped: never imports `Finding` — deliberately distinct from `exclusions.py`, which filters findings. The diff-line size cap lands in a later task. |
