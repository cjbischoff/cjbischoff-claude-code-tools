        ...
        json.dumps(
            {
                ...
                "sast_plan": {},
                "agents_to_spawn": attack_surface,
                "budget_hint": {},
            }
        )
    )
...
def test_findings_json_includes_ndt(tmp_path: Path):
    ...
    write_findings(
        ws,
        [
            _f_new("C-1", FindingStatus.CONFIRMED),
...
def test_report_sections_needs_deployment(tmp_path):
    ...
    ndt = Finding(
        ...
        file="a.py",
        line=3,
        message="needs proxy to confirm",
    )
    ...
    write_findings(
        ...
        [
            Finding(
                ...
                message="m",
                risk_score=5,
                evidence_sources=["ripgrep:a.py:1"],
            )
        ],
    )
    write_report(ws)
...
def test_to_markdown_renders_coverage_ledger():
    ...
    led = {
        ...
        "surfaces": [{"id": "auth", "disposition": "reported"}],
...
def _ndt():
    return Finding(
        ...
        runtime_test={
            "objective": "verify CE-ID isolation on operatorFeedbackWrite",
...
def _confirmed_dep():
    ...
    return Finding(
        ...
        line=1,
        risk_score=3,
        message="decompress@4.2.1: GHSA-x — path traversal",
        evidence="decompress@4.2.1",
        evidence_sources=["sca:osv:GHSA-x"],
        reachability={"reachable": False, "blocker": "dev"},
        id="DEP-T4",
    )
...
def test_collapse_clusters_keeps_one_representative():
    sites = [
        {"id": "F-1", "file": "r_F-1.py", "line": 1},
        {"id": "F-2", "file": "r_F-2.py", "line": 1},
        {"id": "F-3", "file": "r_F-3.py", "line": 1},
    ]
    findings = [
        _ndt_cluster("F-1", "cluster:F-2"),
