...
from sec_overlay.sarif import _SCHEMA, to_sarif
...
def to_correlation_sarif(
    ...
    if none, so coverage is explicit); a final ``sec-overlay-correlation`` run
    ...
    runs.append(
        {
            "tool": {"driver": {"name": "sec-overlay-correlation", "rules": []}},
            "results": results,
        }
    )
