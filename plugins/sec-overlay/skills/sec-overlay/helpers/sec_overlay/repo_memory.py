...
default it lives INSIDE the reviewed codebase at ``<target>/.sec-overlay/<slug>/``
(override the base with ``$SEC_OVERLAY_HOME`` for an external location, or the whole
workspace with ``--workspace``). The harness never executes or modifies the reviewed
SOURCE — its own artifacts live in this self-ignoring ``.sec-overlay/`` sidecar dir,
...
def memory_root(target: str | Path | None = None) -> Path:
    ...
    Resolution precedence:
        1. ``$SEC_OVERLAY_HOME`` if set — an explicit external base.
        2. ``<target>/.sec-overlay`` when ``target`` is given — the default, an
           in-repo sidecar next to the reviewed code.
        3. ``~/.sec-overlay`` as a last resort when no target is known.

    Args:
        target: Path to the reviewed repo. When given (and no env override), the
            base is the in-repo ``.sec-overlay`` sidecar.

    Returns:
        The base directory that will hold ``<slug>/``.
    """
    env = os.environ.get("SEC_OVERLAY_HOME")
    if env:
        return Path(env).expanduser()
    if target is not None:
        return Path(target).expanduser().resolve() / ".sec-overlay"
    return Path.home() / ".sec-overlay"
...
class RepoMemory:
    ...
    def ensure(self, target: str | Path | None = None) -> None:
        ...
        # Make the .sec-overlay sidecar self-ignoring so in-repo scan output never
        ...
        if not sidecar_ignore.exists():
            ...
            sidecar_ignore.write_text("# sec-overlay scan artifacts — not source\n*\n")
        if not self.index_path.exists():
            hdr = [
                f"# sec-overlay memory — {self.root.name}",
                "",
