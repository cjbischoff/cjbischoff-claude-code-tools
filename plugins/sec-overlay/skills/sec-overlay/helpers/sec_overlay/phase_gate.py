...
sec-overlay already battle-tests investigate findings (critic + adversarial-validate). Every
...
def claims_from_profile(profile) -> list[dict]:
    ...
    Args:
        profile: A :class:`sec_overlay.profile.ScanProfile` (or any object exposing the same
...
def claims_from_context(ctx) -> list[dict]:
    ...
    Args:
        ctx: A :class:`sec_overlay.context.Context` (or any object exposing an ``items``
            list of :class:`~sec_overlay.context.ContextItem`-shaped entries).
