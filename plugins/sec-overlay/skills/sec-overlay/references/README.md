...
|------|---------|-----------------|
| `finding.schema.json` | `findings_gate.py` | Every `findings/*.json` must validate: required fields, `status`/`severity` enums, the hard rule that `confirmed`/`fixed` findings carry ≥1 tool receipt, the inner shape of `runtime_test` (its `expected_signal` may be object, string, or null — the renderers tolerate all three), and `open_questions` (array of `{question, why_it_matters, who_to_ask_or_check}` objects — human-answerable unknowns a live-exploit test can't settle, populated by `trace`/`redteam`). |
