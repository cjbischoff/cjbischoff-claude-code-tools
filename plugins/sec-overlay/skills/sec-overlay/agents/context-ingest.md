...
DIAGRAM_STYLE + FIELD_OWNERSHIP from `{{OVERLAY_ROOT}}/references/prompt-constants.md`.
...
components or data flow here. Build it as a mermaid string (fenced ```mermaid``` block)
and set it on the `Context` object's `diagram` field BEFORE calling `save()`;
