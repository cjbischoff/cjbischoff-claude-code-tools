...
1. **No commit in mando's history satisfies D-05's literal diff-selection criteria.**
   ...
   - **RESOLVED (planning phase):** Planner selected a real `app/`-only historical diff (5f477d8..d06ce30, 15 files, all TS/TSX) and recorded the mixing-criterion deviation in 05-01-PLAN.md's flagged assumptions (edge E-12). D-12 disposition applied: documented as a real-output defect for later review, not a blocker.
...
2. **Exact `--config`/rules path for the audit run.**
   ...
   - **RESOLVED (planning phase):** Planner confirmed `/sec-overlay:audit` resolves config path internally; audit tasks in 05-02-PLAN.md and 05-04-PLAN.md invoke the full skill path without explicit `--config` argument. No executor action needed.
