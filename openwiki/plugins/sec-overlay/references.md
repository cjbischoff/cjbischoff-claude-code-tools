...
flowchart LR
    subgraph REF["references/"]
        ...
        STD["architecture/threat-model-standards.md<br/>+ mermaid-caps.md"]
        FT["finding-template.md"]
        HUNT["hunting/*.md"]
        CG["codeguard/*.md"]
        ASVS["asvs/asvs_5.0.0.json"]
        CRYPTO["approved-crypto-*.yaml"]
        DS["dependency-sinks.json<br/>route-frameworks.json"]
        SCHEMA["*.schema.json"]
    ...
    subgraph HELPERS["helpers/sec_overlay/"]
        ...
        DGATE["diagram_gate.py + ste_lint.py"]
        DSPY["dependency_sinks.py + route_census.py"]
    end
    PC -->|injected into prompt text| A
    AC --> A
    STD -->|enforced by| DGATE
    HUNT --> A
    FT -->|rendered by| REP
...
| `GENERAL_PROFILE_EXCLUSION_RULES` | The same five gates for the `general` review profile: gates A and B are skipped for a general-defect class (null-dereference, thread-safety, resource-leak, error-swallowing, injection); gates C/D/E always apply. See [review mode](review-mode.md#the-security-vs-general-profile). |
...
| `TOOL_TRUST` | Mechanical receipts outrank `llm-claimed` reasoning; bytes read through a piped shell are not trustworthy for exact-content claims. An absence check cuts the other way too — an over-rigid rule flags every call site, including fixed ones; confirm it produces no match on a known-safe site before citing an absence. |
...
list beats a guessed one. Every row's ripgrep tokens are pinned to match the corresponding
`dependency-sinks.json` entry's `sink`/`indicators` fields when one exists (regression-tested).

## `architecture-standards.md` and `threat-model-standards.md` — the diagram/document contracts

Two files fix what the architecture and threat-model phases must produce, replacing the
earlier free-form `kb/architecture.md`/`kb/THREAT_MODEL.md` single files:

- **`architecture-standards.md`** — which C4 diagrams `architecture.md` must emit (context,
  container, component-when-complex, runtime-view sequences), the `arc42.md` section table
  (§5 Building Block View replaces the old `kb/entities/` files), and the ownership boundary:
  architecture owns structure and rationale, **never** threats or mitigations.
- **`threat-model-standards.md`** — the per-system methodology-selection rule (STRIDE always;
  PASTA/LINDDUN added only by evidenced signal, never hardcoded), how `dfd.mmd` derives from
  `container-diagram.mmd` (same element ids, trust-boundary `subgraph`s, a `%% derived-from:
  <file> sha256:<hash>` provenance header), and the findings-table column contract (threat, DFD
  element, STRIDE/LINDDUN category, CVSS v4.0, mitigation, residual risk). Threat-model never
  restates architecture narrative — it references `arc42.md` by section instead.

Both diagrams obey **`mermaid-caps.md`**, the single source of truth for per-diagram-kind
node/participant/message ceilings, label word limits, the orphan-node rule (with a store/actor
escape hatch), and the group→split→promote re-scoping order on a cap breach — mirrored
mechanically in `sec_overlay.diagram_gate.CAPS`/`SEQ_CAPS` and kept from drifting by
`tests/test_references_caps.py`. `diagram_gate.py` and `ste_lint.py` (see
[helpers](helpers.md)) are the deterministic gates enforcing all three files; see
[pipeline](pipeline.md#the-full-phase-order) for where `arch-gate`/`tm-gate` run.

**Consumed by:** `agents/architecture.md`, `agents/threat-model.md`, and every agent emitting a
mermaid diagram.
...
| `dependency-sinks.json` | `dependency_sinks.py` | dependencies whose own code holds the sink (e.g. OPA's `http.send`) — no first-party pattern exists for these, so recon routing and the recall gate rely on a manifest match instead of a code pattern |
