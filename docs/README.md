...
| `superpowers/specs/2026-08-22-sec-overlay-recall-gaps-design.md` | Design for six sec-overlay recall features from the OPA/Rego SSRF gap analysis (G6–G12), each verified against live source with `file:line` citations: a missing-safe-option semgrep pack plus ast-grep relational constraints, a recall adversary that reports omissions, a dependency-sink catalog that drives recon routing, catalog-gated proof tuples for dependency-internal sinks, server-side policy-engine indicators, and a code-derived route census with sink-site coverage keying |
