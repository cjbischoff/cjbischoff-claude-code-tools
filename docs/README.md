...
| `superpowers/specs/2026-08-16-sec-overlay-invocation-design.md` | Design for a driven sec-overlay invocation: one `/sec-overlay:audit` command (one repo → audit; N repos → audit each then correlate), a `run.py` driver (token env, per-phase receipts, working-tree fence), scan-profile role inference feeding the correlation core, and the O-65 one-writer path fix; coverage/recall family deferred |
